//
//  ConnectRecoverySession.swift
//  ConnectSDK
//
//  Manages recovery session configuration and presentation
//

import UIKit

/// A configured recovery session that can be presented when ready
@MainActor
public class ConnectRecoverySession {
    ...
    internal init(jwt: String, environment: Environment, theme: Theme, callbacks: RecoveryCallbacks, allowList: ConnectAllowList = .default, oauthCallback: ConnectOAuthCallback = .default) {
        self.jwt = jwt
    ...
    /// Presents the recovery UI from the specified view controller
    ...
    public func present(from viewController: UIViewController) -> ConnectSession? {
        ...
        let callbackHandler = RecoveryCallbackHandler(callbacks: callbacks)

        let webVC = WebViewController(
            urlString: ConnectApp.recovery.baseURL(for: environment),
            jwt: jwt,
        ...
        let session = ConnectSession(app: .recovery, viewController: nav)
